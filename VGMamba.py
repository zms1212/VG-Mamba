...
# from models.mamba_net import Mamba
from models.VisionInspired import VisionInspired
from models.SemAssociationEmbed import SAEmbed
...
class VIMamba(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, c_list=[8,16,24,32,48,64],
        ...
        self.encoder1 = nn.Sequential(
            ...
            VisionInspired(in_planes=c_list[0], out_planes=c_list[0], map_reduce=2),
        )

        self.encoder2 = nn.Sequential(
            nn.Conv2d(c_list[0], c_list[1], 3, stride=1, padding=1),
            VisionInspired(in_planes=c_list[1], out_planes=c_list[1])
        )

        self.encoder3 = nn.Sequential(
            nn.Conv2d(c_list[1], c_list[2], 3, stride=1, padding=1),
            VisionInspired(in_planes=c_list[2], out_planes=c_list[2])
        )
        ...
        # 语义嵌入
        self.saembed5 = SAEmbed(channels=48, out_channels=48)
        self.saembed4 = SAEmbed(channels=32, out_channels=32)
        self.saembed3 = SAEmbed(channels=24, out_channels=24, r=8)
        self.saembed2 = SAEmbed(channels=16, out_channels=16, r=8)
        self.saembed1 = SAEmbed(channels=8, out_channels=8, r=4)
        ...
        self.outconv4 = nn.Conv2d(c_list[3], 1, 1, 1, 0)
        self.outconv3 = nn.Conv2d(c_list[2], 1, 1, 1, 0)
        self.outconv2 = nn.Conv2d(c_list[1], 1, 1, 1, 0)
        self.outconv1 = nn.Conv2d(c_list[0], 1, 1, 1, 0)
        self.sigmoid = nn.Sigmoid()
